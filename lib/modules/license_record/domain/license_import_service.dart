...
class LicenseImportService {
  ...
  Future<LicenseImportResult> importExistingRecords(File file) async {
    ...
    _assertHeaders(sheet: sheet, headers: existingRecordHeaders);
      ...
      try {
        ...
        switch (operation) {
          case _ImportOperation.insert:
            ...
            final LicenseRecordEntity entity = _parseExistingRecordRow(row);
  ...
  Future<LicenseImportResult> batchGenerateFromFile(File file) async {
    final LicenseRecordService service =
        _recordService ??
        LicenseRecordService(repository: _repository, now: _now);
    _assertTemplateFileName(
      file,
      requiredBaseName: batchGenerateTemplateFileBaseName,
    );
    final Sheet sheet = await _loadSheet(file);
    _assertHeaders(sheet: sheet, headers: batchGenerateHeaders);
    final List<List<Data?>> dataRows = _dataRows(sheet);
    _assertMaxImportRows(dataRows.length);
    ...
    for (int index = 0; index < dataRows.length; index++) {
      ...
      try {
        final _BatchGenerateInput input = _parseBatchGenerateRow(row);
  ...
  void _assertHeaders({required Sheet sheet, required List<String> headers}) {
    final List<String> actualHeaders = _normalizeHeaderCells(
      sheet.row(0),
    ).map((Data? cell) => _cellText(cell)).toList(growable: false);
    if (_listEquals(actualHeaders, headers)) {
      return;
    }
...
enum _ImportOperation { insert, update, delete }

class _DurationValue {
  const _DurationValue({this.days, required this.permanent});
